...
while True:
    try:
        ...
        if changes :
            ...
            data_pm25 = f"PM25;{round(pm25,2)};{round(flow,2)};END_PM25;"
            db.update_sensor_values(1, 1, data_pm25)
            data_pm10 = f"PM10;{round(pm10,2)};{round(flow,2)};END_PM10;"
            db.update_sensor_values(1, 2, data_pm10)
            print("Read PM")
            time.sleep(1.2)
            ...
            # db.update_sensor_values(1,31,f"SEMEATECH_DATA;1;NO2;{no2};{ppb_no2};{round(random.uniform(22, 25), 2)};{round(random.uniform(63, 65), 2)}")
        
            # so2 = int(random.uniform(20,30)) # 2
            # ppb_so2 = int(so2 * 0.376344086021505)
            # db.update_sensor_values(1,32,f"SEMEATECH_DATA;2;SO2;{so2};{ppb_so2};{round(random.uniform(22, 25), 2)};{round(random.uniform(63, 65), 2)}")
            # ppb_so2 = so2 * 2.475122349102773

            # o3 = int(random.uniform(6,10) )# 3
            # ppb_o3 = int(o3 * 0.506849315068493)
            # db.update_sensor_values(1,33,f"SEMEATECH_DATA;2;O3;{o3};{ppb_o3};{round(random.uniform(22, 25), 2)};{round(random.uniform(63, 65), 2)}")
