...
try:
    ...
    print("[V] Semeatech Miniboard Connected")
except Exception as e: 
    print("[X] Semeatech Miniboard Sensor ID: " + str(sys.argv[1]) + " " + e)
    ser = None
    ...
    try:
        ...
        if(ser.in_waiting > 0):
            ...
            if("END;" in response):
                ...
                pin = response.split(" ")[2][2]           
                update_sensor_value(str(sys.argv[1]), pin, value)
    except Exception as e:
        print("Error Reading Sensor: ",e)
        value = "0x0;NULL;0;0;0;0;SEMEATECH"
        update_sensor_value(str(sys.argv[1]), "1", value)
        update_sensor_value(str(sys.argv[1]), "2", value)
        ser.close()
    time.sleep(1)