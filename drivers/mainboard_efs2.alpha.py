...
def execute_command(sensor_reader_id, pin, data, prefix_return_batch=None):
    try:
        hash_function = {"single": store_data_single, "batch": store_data_batch}
...
def get_data_from_motherboard(type):
    try:
        ...
        cursor.execute(
            "SELECT * FROM motherboard where type=%s and is_enable = 1 order by is_priority desc",
            (type,),
        )
