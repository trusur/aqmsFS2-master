...
class Sentdata_ws extends BaseCommand
{
	...
	public function run(array $params)
	{
		...
		if ($is_sentto_trusur == "1") {
			...
			$time_group = @$this->measurement_logs->orderBy("id", "DESC")->findAll()[0]->xtimestamp;
			if ($time_group) {
				$is_exist = true;
				$arr["waktu"] = date('Y-m-d H:i:00');
				$measurements = @$this->measurement_logs->where(["DATE_FORMAT(xtimestamp, '%Y-%m-%d %H:%i')" => date('Y-m-d H:i', strtotime($time_group))])->orderBy("id", "desc")->findAll();
